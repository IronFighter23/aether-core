...
export interface AetherOptions {
  ...
  cacheKey?: string;
  /**
   * Shared-secret authentication token. When provided, the value is
   * delivered to the gateway both as a ``?auth_token=...`` query
   * parameter on the WebSocket URL AND as a first-frame
   * ``{type:'auth',token:...}`` message after open. The gateway
   * accepts whichever arrives first. Required when the gateway was
   * constructed with ``AuthConfig(token=...)``.
   */
  authToken?: string;
}

/** Callback fired when a local write loses the HLC race against a remote one. */
export type SupersedeWatcher = (
  key: string,
  attemptedValue: AetherValue | undefined,
  actualValue:   AetherValue | undefined,
) => void;
...
export class Aether {
  ...
  /**
   * Subscribe to "your write was superseded" events. Fires when a
   * value you wrote via ``set(k, v)`` (or implicit-delete via
   * ``delete(k)``) loses the LWW race against a concurrent writer
   * for the same key, within a 10-second window. The callback
   * receives ``(key, attemptedValue, actualValue)``.
   *
   * This is the visibility layer for LWW conflict resolution: the
   * CRDT does not silently drop writes -- every op is recorded and
   * federated -- but the math may choose someone else's value as
   * the final one. Subscribe here when your UI needs to react.
   */
  onSupersede(callback: SupersedeWatcher): Unsubscribe;
