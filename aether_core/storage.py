"""
Aether-Core :: Chrono-Vector Storage
====================================

Data is not stored as state; it is stored as an immutable, append-only
ledger of state-change vectors. The ledger is a JSON-lines file written
with single-syscall ``os.write()`` calls on an ``O_APPEND`` file
descriptor, fsync'd after each record. This gives two guarantees:

1. **Atomicity per record.** Records small enough to fit in PIPE_BUF
   (>= 4096 bytes; ours are well under) are appended atomically by the
   kernel. A concurrent or interrupted writer cannot interleave bytes
   inside a record.

2. **Bounded corruption surface.** If the process is killed mid-write,
   the *only* damaged region is a torn final line. The replay routine
   detects "no trailing newline" on the last record and truncates the
   file to the last clean newline. Every fully-flushed record before
   the crash survives.
...
``Node.receive()`` reproduces *exactly* the post-crash state including
tombstones.
"""
...
class ChronoLedger:
    ...
    async def boot(self, mesh_node: MeshNode) -> int:
        ...
        # Step 1: replay + crash recovery, all on a worker thread so we
        # don't stall the event loop on large ledgers.
        ...
        # write() goes to the current end of file even if another writer
        # extends it concurrently. O_CREAT creates if missing.
        ...
        # Step 3: spin up the writer task.
    ...
    def _write_sync(self, line: bytes) -> None:
        """
        Atomic per-record append. Single os.write() syscall under
        O_APPEND is kernel-guaranteed atomic for records < PIPE_BUF.
        fsync gives durability across power loss.
        """
        assert self._fd is not None
        # write() may legally write fewer bytes than requested; loop
        # until the buffer is exhausted. In practice for a JSONL line
        # to a regular file this always completes in one call.
...
async def _demo() -> None:
    ...
    # Deliberately do NOT call mesh_b.start() and connect no peers.
    # Reconstruction must be entirely from the ledger.
    ...
    # Specifically prove the tombstone survived.
    ...
    # Append a half-written record (no terminating newline). This is
    # exactly what an OS-level SIGKILL between os.write() calls would
    # leave behind in the worst case.
